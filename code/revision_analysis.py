...
class Analyze:
	...
	def __init__(self, data, language, topic, daily=False):
		self.data = data
		self.language = language
		self.topic = topic
		self.daily_data, self.timestamps, self.dates = self._one_per_timestamp(daily)
		
	def _timestamp2date(self, timestamp, daily=False):
	...
	def _one_per_timestamp(self, daily=False):
	...
	def _remove_vandalism(self, added_elements, removed_elements):
		...
		for n, date in enumerate(removed_elements.keys()):
			if n >= len(self.dates)-1: continue
			if removed_elements[date] == added_elements[self.dates[n+1]] and len(removed_elements[date]) != 0:
				removed_elements[date] = []
				added_elements[self.dates[n+1]] = []
	...
	def list_development(self, element_type, remove_vandalism=False, visualize=False):
		...
		if remove_vandalism:
			...
			added_elements, removed_elements = self._remove_vandalism(self.added_elements, self.removed_elements)
